...
export default function Navbar({ onLogin, onViewDashboard, onRequestQuote, settings }: NavbarProps) {
  ...
  const navItems: NavMenuItem[] = settings?.navItems?.length
    ...
    : [
        ...
        { id: 'footer', label: 'Contact', url: '#footer' },
      ];
        ...
        <div className={`flex justify-between items-center transition-all duration-300 ${isScrolled ? 'h-16' : 'h-20'}`}>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <button
              ...
              className={`btn-studio px-8 py-3 text-[10px] font-black uppercase tracking-[0.2em] transition-all cursor-pointer ${
                headerIsTransparent
            ...
            >
              {settings?.buttonText || 'Request a Quote'}
            </button>
          </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-[#FDFCFB] border-b border-black/10 overflow-hidden"
          >
            <div className="px-8 py-10 space-y-6">
              {navItems.map((item) => (
                <div key={item.id}>
                  <a
                    ...
                    rel={item.openInNewTab ? 'noreferrer' : undefined}
                    onClick={closeMenu}
                    className="block text-2xl font-display font-black uppercase tracking-tight text-black hover:text-[#2D545E]"
                  >
                    {item.label || 'Menu item'}
                  </a>
                </div>
              ...
              <button
                ...
                className="w-full bg-black text-white py-5 text-[12px] font-black uppercase tracking-[0.3em] hover:bg-[#2D545E] text-center cursor-pointer transition-colors"
              >
                {settings?.buttonText || 'Request a Quote'}
              </button>
            </div>
